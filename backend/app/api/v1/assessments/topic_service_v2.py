...
async def generate_questions_for_row_v2(
    ...
    # Generate questions based on normalized type
    # Note: experience_mode can be passed if available, defaulting to "corporate"
    experience_mode = "corporate"  # Can be enhanced to pass from assessment settings
    
    if question_type_normalized == "MCQ":
        return await _generate_mcq_questions(topic_label, difficulty, questions_count)
    elif question_type_normalized == "Subjective":
        return await _generate_subjective_questions(topic_label, difficulty, questions_count, experience_mode)
    elif question_type_normalized == "PseudoCode":
        return await _generate_pseudocode_questions(topic_label, difficulty, questions_count)
    elif question_type_normalized == "Coding":
        return await _generate_coding_questions(topic_label, difficulty, questions_count, can_use_judge0, coding_language)
    else:
...
async def _generate_mcq_questions(topic: str, difficulty: str, count: int) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        response = await client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert assessment writer. Always return valid JSON arrays. Never include markdown code blocks."
                },
                {"role": "user", "content": prompt}
            ],
            temperature=0.7,
