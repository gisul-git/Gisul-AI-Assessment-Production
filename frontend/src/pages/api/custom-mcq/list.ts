...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    try {
      const response = await fastApiClient.get("/api/v1/custom-mcq/list", {
        headers: {
      ...
      if (error?.response?.status === 401 && refreshToken) {
        ...
        if (newToken) {
          ...
          try {
            const retryResponse = await fastApiClient.get("/api/v1/custom-mcq/list", {
              headers: {
            ...
            const errorMessage =
              ...
              retryError?.message ||
              "Failed to fetch custom MCQ assessments";
    ...
    console.error("Error fetching custom MCQ assessments:", error);
