...
export default function DSATestVerifyPage() {
  ...
  useEffect(() => {
    ...
    const verifyLink = async () => {
      try {
        const response = await axios.get(`${apiUrl}/api/v1/dsa/tests/${testId}/verify-link?token=${encodeURIComponent(token)}`);
        setUserInfo({
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Start the test
      await axios.post(`${apiUrl}/api/v1/dsa/tests/${testId}/start?user_id=${userInfo.user_id}`);
