...
export function useLiveProctorAdmin({
  ...
  const createSession = useCallback(
    async (assessmentId: string, candidateId: string, adminId: string) => {
      try {
        ...
        const allSessionsRes = await fetch(`${API_URL}/api/v1/proctor/live/all-sessions/${assessmentId}`);
        ...
        if (existingSession) {
          ...
          const createRes = await fetch(`${API_URL}/api/v1/proctor/live/create-session`, {
            method: "POST",
  ...
  const setupPeerConnection = useCallback(
    async (sessId: string, session: LiveProctorSession) => {
      ...
      try {
        ...
        // Send answer
        await fetch(`${API_URL}/api/v1/proctor/live/answer`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            sessionId: sessId,
  ...
  const disconnect = useCallback(() => {
    ...
    if (sessionId) {
      fetch(`${API_URL}/api/v1/proctor/live/end-session/${sessionId}`, {
        method: "POST",
      }).catch(() => {});
    }
